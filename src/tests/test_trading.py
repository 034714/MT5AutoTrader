...
PASSED: list[str] = []
FAILED: list[str] = []
VERBOSE = False   # -v 时显示每个 PASS，默认只报失败（跑分域时输出更清爽）
...
#
# 用法（分域按需跑，不必每次全量）：
#   python tests/test_trading.py              # 全量（发版/强推前）
...
def main() -> int:
    ...
    if not args:
        fns = [f for name, domain in DOMAINS.items() if name != "quick" for f in domain]
        label = "全量"
    else:
        fns = []
        for a in args:
            if a not in DOMAINS:
                print(f"未知分组 {a!r}，可选：{', '.join(DOMAINS)}")
                return 2
            fns.extend(DOMAINS[a])
            label = "+".join(args)
    print("=" * 64)
    print(f"  MT5AutoTrader 测试 [{label}]（全部 mock，不触碰真实 MT5/订单）")
    print("=" * 64)
    for fn in fns:
        try:
            fn()
