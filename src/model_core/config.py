...
class ModelConfig:
    ...
    # 多岛并行：同一迁移阶段内各岛在独立线程同时训练。2026-09-12 实测（3 岛
    # ×10 步，Ryzen 9 5900HX）：串行 678s vs 并行 618s，仅快 9%——单岛训练时
    # torch intra-op 线程已吃满全部核心，并行只是共享饱和的核心。默认关。
    ISLAND_PARALLEL:        bool  = False
