...
class KlineCache:
    ...
    def _full_download(self, symbol: str) -> Optional[pd.DataFrame]:
        ...
        MT5 copy_rates_from_pos 单次最多约 5 万根（再多返回 Invalid params），
        因此按 MAX_CHUNK 分块向后翻页拉取，直到达到 bars_count 或数据取完。
        """
        if not _MT5_AVAILABLE or mt5 is None:
            return None
        MAX_CHUNK = 50000
        logger.info(f"[Cache] {symbol}: 全量下载（上限 {self.bars_count} bars，分块拉取）...")
        t0 = time.time()
        parts: list[pd.DataFrame] = []
        pos = 0
        while len(parts) == 0 or len(parts[-1]) >= MAX_CHUNK:
            if sum(len(p) for p in parts) >= self.bars_count:
                break
            rates = mt5.copy_rates_from_pos(symbol, self.timeframe, pos, MAX_CHUNK)
