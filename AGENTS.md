...
- **Do NOT run the full suite after every task.** Tests live in `src/tests/test_trading.py` and are domain-filtered:
  ...
  - `... runner` — 交易流程/台账/策略加载
  - `... sr` — 支撑/阻力、止盈一半、手动覆盖
  - `... signal` — 信号阈值 + 真实策略端到端（较慢）
  - `... quick` — 秒级核心冒烟
  - no arg = 全量，仅发版/强推/大重构前跑
- Default output is quiet (failures only); add `-v` to see every PASS.
- 网页（index.html）/文档/构建脚本改动：做静态检查或浏览器验证即可，不跑单测。
- New features still add tests to the right domain — coverage stays, ritual shrinks.
